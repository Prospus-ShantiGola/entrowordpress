...
if ( ! class_exists('CCB_Shortcode') ) {
    ...
    class CCB_Shortcode extends CCB_Module {
        ...
        protected static function create_button_options( $config ) {
            if ( strcmp( $config['output'], 'blob' ) == 0 ) {
            ...
            $options = 'var options' . self::$id . ' = ' . json_encode( $config, JSON_NUMERIC_CHECK ) . ';';
        ...
        public static function render_shortcode( $attributes = array() ) {
            ...
            $attributes = apply_filters( 'ccb_shortcode-attributes', self::parse_button_atts( $attributes, self::$settings ) );
