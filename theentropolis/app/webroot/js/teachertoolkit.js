...
//$(window).load(function () {
    $('#btnSearch').click(function () {
        ...
        $.ajax({
            url: "TeacherToolkits/search_files/1",
            data: postForm,
            type: 'POST',
    ...
    $('#btnSearchEdit').click(function () {
        ...
        $.ajax({
            url: "../TeacherToolkits/search_files",
            data: postForm,
            type: 'POST',
            success: function (data) {
                if (data.result == "error") {
    ...
    var addFormValidation = $('#ToolkitAddForm').validate(
            {
                ...
                rules: {
                    'data[TeacherToolkit][name]': {
                        required: true,
                        maxlength: 200
                    },
                    'data[TeacherToolkit][short_description]': {
                        maxlength: 200
                    },
                    'data[TeacherToolkit][description]': {
                        maxlength: 400
                    }
                },
        ...
        buttons: {
            success: {
                ...
                callback: function () {
                    $.ajax({
                        type: 'POST',
                        url: 'TeacherToolkits/delete/' + folder_id,
                        success: function (data) {
                            if (data.result == "error") {
                                bootbox.alert({
                                    title: "Error!!",
...
var fileDirectoryModule = (function () {
    ...
    return {
        ...
        editFolder: function () {
            ...
            $('#ToolkitEditForm [name="data[TeacherToolkit][id]"]').val(input.data("id"));
            $('#ToolkitEditForm [name="data[TeacherToolkit][name]"]').val(folderName);
            $('#ToolkitEditForm #TeacherToolkitShortDescription').val(folderTitle);
        },
        editDescription: function () {
            ...
            $('#EditDescPopup #TeacherToolkitDescription').val(folderDesc);
        },
        editFile: function (selector) {
            ...
            $('#ToolkitEditFileForm [name="data[TeacherToolkit][id]"]').val(currentElm.data("id"));
            $('#ToolkitEditFileForm [name="data[TeacherToolkit][old_name]"]').val(fileName);
            $('#ToolkitEditFileForm [name="data[TeacherToolkit][title]"]').val(fileName);
            $('#ToolkitEditFileForm #TeacherToolkitShortDescription').val(fileTitle);
        },
...
/*$("#upload_files").submit(function () {
 ...
 $.ajax({
 url: 'teachertoolkits/add_file', // point to server-side PHP script 
        ...
        buttons: {
            success: {
                ...
                callback: function () {
                    $.ajax({
                        type: 'POST',
                        url: '../TeacherToolkits/delete_file/',
                        data: postForm,
                        success: function (data) {
                            if (data.result == "error") {
                                bootbox.alert({
...
$("#ToolkitEditDescription").submit(function () {
    ...
    $.ajax({
        url: "../TeacherToolkits/edit_description",
        data: datas,
        type: 'POST',
        success: function (data) {
            if (data.result == "error") {
...
// form validation of video upload
    var validatorVideo = $('#ToolkitAddVideoForm').validate(
            {
                errorClass: "fileupload-error-fields",
                rules: {
                    'data[TeacherToolkit][video_url]': {
                        required: true,
                        url: true
                    },
                    'data[TeacherToolkit][title]': {
                        required: true
                    }
                },
                submitHandler: function (form) {
    ...
    var editFolderValidator = $('#ToolkitEditForm').validate({
        ...
        submitHandler: function (form) {
            ...
            var datas = $('#ToolkitEditForm').serialize();
            $.ajax({
                url: "../TeacherToolkits/edit_folder",
                data: datas,
                type: 'POST',
                success: function (data) {
                    if (data.result == "error") {
