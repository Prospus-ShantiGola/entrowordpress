...
if(file_exists(dirname(dirname(dirname(__FILE__))).DIRECTORY_SEPARATOR."config".DIRECTORY_SEPARATOR."app.php")) {
	...
	define('CAKEPHP_VERSION', '3.x.x');
        ...
        include_once(dirname(dirname(dirname(__FILE__))).DIRECTORY_SEPARATOR."Config".DIRECTORY_SEPARATOR."site-setting.php");
...
define('DB_PASSWORD',		't#3en+r0'		); // Database password
define('DB_NAME',			'trepicity'		); // Database name

$table_prefix = $config->default['prefix'];									// Table prefix(if any)
...
class Integration{
	...
	function getFriendsIds($userid) {
		...
		$sql = ("SELECT inviter_user_id as friendid FROM `invitations` WHERE invitation_status =1 and invitee_user_id = '".sql_real_escape_string($userid)."' union SELECT invitee_user_id as myfrndids FROM `invitations` WHERE invitation_status = 1 and inviter_user_id = '".sql_real_escape_string($userid)."'");

		return $sql;
	}

	function getUserDetails($userid) {
		$sql = ("select ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." userid,".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." user_image, ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_NAME." username, ".DB_LINKFIELD." link, ".DB_AVATARFIELD." avatar, cometchat_status.lastactivity lastactivity, cometchat_status.lastseen lastseen, cometchat_status.lastseensetting lastseensetting, cometchat_status.status, cometchat_status.message, cometchat_status.isdevice, cometchat_status.readreceiptsetting readreceiptsetting from ".TABLE_PREFIX.DB_USERTABLE." left join cometchat_status on ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." = cometchat_status.userid ".DB_AVATARTABLE." where ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." = '".sql_real_escape_string($userid)."'");
	...
	function getAvatar($image) {
            if ($image) {
         return $_SERVER['http_host']."/app/webroot/".$image;
    } else {    
