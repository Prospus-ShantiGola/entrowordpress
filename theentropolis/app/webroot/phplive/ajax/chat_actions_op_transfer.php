<?php
	...
	else if ( $action === "transfer" )
	{
		...
		$lang = $CONF["lang"] ;
		$opinfo = Ops_get_OpInfoByID( $dbh, $opid ) ; $profile_src = "" ;
		...
		$text = "<idle_pause><div class='ca'>".$LANG["CHAT_TRANSFER"]." <b><top>$opname  ($deptinfo[name])</top><!--opid:$opinfo[opID]--><!--mapp:$opinfo[mapp]--><!--name:$opname--><!--profile_pic:$profile_src--><!--department:$deptinfo[name]--></b></b>. <div style='margin-top: 10px;'><div class='ctitle'> ".$LANG["TXT_CONNECTING"]."</div></div></div></idle_pause>" ;

		Chat_update_TransferChat( $dbh, $ces, $opid_cookie, $deptid, $opid, $now+$deptinfo["rtime"] ) ;
