<?php
					...
					$lang )
	{
		...
		LIST( $deptid, $email, $emailt, $emailt_bcc, $visible, $queue, $rtype, $rtime, $rloop, $savem, $vupload, $ctimer, $custom, $smtp, $tshare, $texpire, $temail, $lang, $msg_greet, $msg_offline, $msg_busy, $transcript ) = database_mysql_quote( $dbh, $deptid, $email, $emailt, $emailt_bcc, $visible, $queue, $rtype, $rtime, $rloop, $savem, $vupload, $ctimer, $custom, $smtp, $tshare, $texpire, $temail, $lang, $msg_greet, $msg_busy, $msg_offline, $msg_transcript ) ;

		$query = "INSERT INTO p_departments VALUES ( $deptid, $visible, $queue, $tshare, $texpire, 1, 1, $temail, $temaild, $rtype, $rtime, $rloop, $savem, '$vupload', $ctimer, '$custom', '$smtp', '$lang', '$name', '$email', '', '$emailt', $emailt_bcc, '$msg_greet', '$msg_offline', '$msg_busy', '$transcript' ) ON DUPLICATE KEY UPDATE name = '$name', email = '$email', visible = $visible, queue = $queue, rtype = $rtype, rtime = $rtime, rloop = $rloop, savem = $savem, vupload = '$vupload', ctimer = $ctimer, tshare = $tshare, texpire = $texpire, lang = '$lang'" ;
		database_mysql_query( $dbh, $query ) ;
