<?php
					...
					$upload )
	{
		...
		LIST( $opid, $status, $mapper, $rate, $sms, $op2op, $traffic, $viewip, $nchats, $maxc, $maxco, $password, $name, $email, $tag, $upload ) = database_mysql_quote( $dbh, $opid, $status, $mapper, $rate, $sms, $op2op, $traffic, $viewip, $nchats, $maxc, $maxco, $password, $name, $email, $tag, $upload ) ;

		if ( isset( $operator["opID"] ) )
			$query = "UPDATE p_operators SET mapper = $mapper, rate = $rate, op2op = $op2op, traffic = $traffic, viewip = $viewip, nchats = $nchats, maxc = $maxc, maxco = $maxco, tag = $tag, upload = '$upload', sms = $sms, login = '$login', password = '$password', name = '$name', email = '$email' WHERE opID = $opid" ;
		else
			$query = "INSERT INTO p_operators VALUES ( $opid, 0, 0, 0, $mapper, 0, 0, $rate, $op2op, $traffic, $viewip, $nchats, $maxc, $maxco, 0, $tag, '$upload', '', '', 0, $sms, '', '$login', '$password', '$name', '$email', 1, 'default' )" ;
					...
					$status )
	{
		...
		$query = "INSERT IGNORE INTO p_dept_ops VALUES ( $deptid, $opid, $display, $visible, $status )" ;
		database_mysql_query( $dbh, $query ) ;
