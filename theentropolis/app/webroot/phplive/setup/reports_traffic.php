...
<?php include_once( "./inc_header.php" ) ?>
		...
		<div id="foot_settings" style="margin-top: 25px; text-align: justify;">
			<div style="float: left; min-height: 240px; width: 45%" class="info_info">
				<div style="font-size: 14px; font-weight: bold;"><a name="footprints">Visitor Footprint Logging</a></div>
				
				<div style="margin-top: 15px;">
					(default is On) On pages that has the <a href="code.php">Standard or Text Link HTML Code</a>, the system will ping the server every <?php echo $VARS_JS_FOOTPRINT_CHECK ?> seconds to update the visitor's footprint data.  The resource usage of the communication is very minimal and takes milliseconds to process.  However, to reduce server resource usage, switch the setting to Off.
					
					<div class="info_neutral" style="margin-top: 10px;">
						<ul>
							<li> <b>Off</b> will communicate to the server only once, at time of page load
							<li> <b>Off</b> will pause all storing of visitor footprint data
							<li> <b>Off</b> will hide all footprint instances throughout the operator console
						</ul>
					</div>
				</div>

				<div style="margin-top: 25px;">
					<div class="info_good" style="float: left; width: 60px; padding: 3px; cursor: pointer;" onclick="$('#r_foot_settings_on').prop('checked', true);update_foot_log('on');"><input type="radio" name="r_foot_settings" id="r_foot_settings_on" value="on" <?php echo $footprint_on ?>> On</div>
					<div class="info_error" style="float: left; margin-left: 10px; width: 60px; padding: 3px; cursor: pointer;" onclick="$('#r_foot_settings_off').prop('checked', true);update_foot_log('off');"><input type="radio" name="r_foot_settings" id="r_foot_settings_off" value="off" <?php echo $footprint_off ?>> Off</div>
					<div style="clear: both;"></div>
				</div>
			</div>
			<div style="float: left; margin-left: 2px; min-height: 240px; width: 45%;" class="info_info">
				<div style="font-size: 14px; font-weight: bold;">Chat Icon Status Check</div>

				<div style="margin-top: 15px;">
					(default is On) On pages that has the <a href="code.php">Standard or Text Link HTML Code</a>, while the visitor is viewing the page, the system will ping the server every <?php echo $VARS_JS_CHATICON_CHECK ?> seconds to update and gather various information.  The resource usage of the communication is very minimal and takes milliseconds to process.  However, to reduce server resource usage, switch the setting to Off.
					
					<div class="info_neutral" style="margin-top: 10px;">
						<ul>
							<li> <b>Off</b> will switch off the dynamic chat icon image update to online/offline while the visitor is on the same page.
							<li> <b>Off</b> will switch off the ability for operators to chat invite with the visitor.
						</ul>
					</div>
				</div>

				<div style="margin-top: 25px;">
					<div class="info_good" style="float: left; width: 60px; padding: 3px; cursor: pointer;" onclick="$('#r_foot_icon_on').prop('checked', true);update_icon_check('on');"><input type="radio" name="r_foot_icon" id="r_foot_icon_on" value="on" <?php echo $icon_check_on ?>> On</div>
					<div class="info_error" style="float: left; margin-left: 10px; width: 60px; padding: 3px; cursor: pointer;" onclick="$('#r_foot_icon_off').prop('checked', true);update_icon_check('off');"><input type="radio" name="r_foot_icon" id="r_foot_icon_off" value="off" <?php echo $icon_check_off ?>> Off</div>
					<div style="clear: both;"></div>
				</div>
			</div>
			<div style="clear: both;"></div>
