...
<?php include_once( "./inc_header.php" ) ; ?>
		...
		<div id="op_activity" style="display: none; margin: 0 auto;">
			<img src="../pics/icons/bulb_big.png" width="48" height="48" border="0" alt=""> Days with online activities are displayed.
			...
			<div id="reports" style="margin-top: 25px;">
				...
				<tr>
					<td width="50%" valign="top">
						...
						<?php
							$overall_duration = null ; $prev_created = 0 ; $status_text = "" ;
							for ( $c = 1; $c <= $stat_end_day; ++$c )
							{
								...
								if ( count( $reports ) > 0 )
								{
									...
									if ( $c == 1 )
									{
										...
										$prev_day_status = Chat_ext_get_OpStatusPrevOne( $dbh, $opinfo["opID"], mktime( 0, 0, 0, $m, $c, $y ) ) ;
									...
									for( $c2 = 0; $c2 < count( $reports ) ; ++$c2 )
									{
										...
										$diff_string = ( $diff && $debug ) ? "<span class=\"info_blue\">".Util_Format_Duration($diff)."</span>" : "" ;

										$output .= "<div class=\"$class\">$status <span style=\"padding-left: 25px; text-shadow: none;$style\" class=\"info_clear\">$created_expand - $status_text $diff_string</span></div>" ;
									}
					...
					<td width="50%" valign="top">
						...
						<div id="div_clone" style="max-height: 425px; overflow: auto;"></div>
					</td>
				</tr>
				</table>
			</div>
