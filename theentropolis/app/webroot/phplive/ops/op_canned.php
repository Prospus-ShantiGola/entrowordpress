...
<!--
	...
	$(document).ready(function()
	{
		...
		if ( <?php echo $canid ?> )
		{
			...
			$('#canned_container').animate({
				...
				if ( scroll_to ) {
					$('#tr_div_'+<?php echo $canid ?>).fadeOut("fast").fadeIn("fast").fadeOut("fast").fadeIn("fast").fadeOut("fast").fadeIn("fast").promise( ).done(function( ) {
						//
					}) ;
				}
			});
	...
	function toggle_new( theflag )
	{
		...
		else
		{
			...
			$('#canned_box_new').show() ; $('#table_canned_new').show() ;
		}
	}
	...
	function do_delete( thiscanid )
	{
		if ( confirm( "Really delete this canned response?" ) )
			location.href = "op_canned.php?action=delete&canid="+thiscanid ;
	}
	...
	function toggle_vis_idle( theflag )
	{
		if ( $('#table_vis_idle').is(':visible') && !theflag )
		{
			$('#canned_box_new').hide() ; $('#table_vis_idle').hide() ;
			$('#canned_container').show() ;
			toggle_menu_info( "list" ) ;
		}
		else
		{
			$('#canned_container').hide() ;
			$('#canned_box_new').show() ; $('#table_vis_idle').show() ;
		}
	}

	function do_submit_vis_idle()
	{
		var canid_1 = parseInt( $('#canned_select_1').val() ) ;
		var canid_2 = parseInt( $('#canned_select_2').val() ) ;
		var idle = parseInt( $('#vis_idle_minutes').val() ) ;
		return true ; // to be completed for future version
		...
		else
		{
			...
			type: "POST",
			url: "../ajax/chat_actions_op_cans.php",
			data: "action=vis_idle&canid_1="+canid_1+"&canid_2="+canid_2+"&idle="+idle+"&unique="+unique,
			success: function(data){
				eval( data ) ;

				if ( json_data.status )
				{
					parent.vis_idle_canned['canid_1'] = canid_1 ;
					parent.vis_idle_canned['canid_2'] = canid_2 ;
					parent.vis_idle_canned['idle'] = idle ;
					do_alert( 1, "Success" ) ;
				}
				else
					do_alert( 0, "Error updating.  Please refresh the console and try again." ) ;
...
<div id="canned_container" style="padding: 15px; height: 200px; overflow: auto;">
	<div style="padding-bottom: 45px;">
		<div style="padding-bottom: 15px;">
			...
			<div class="menu_traffic_info_focus" style="display: none; font-weight: normal; border: 0px; cursor: default;"><span class="chat_info_td_t" onClick="toggle_vis_idle(1)" style="cursor: pointer;"><img src="../themes/<?php echo $opinfo["theme"] ?>/timer.png" width="12" height="12" border="0" alt=""> Visitor Idle</span></div>
...
<div id="canned_box_new" style="display: none; padding: 15px; height: 200px; overflow: auto;">
	...
	<tr>
		<td valign="top" nowrap width="100%">
			<div style="display: inline-block;" class="info_box"><img src="../themes/<?php echo $opinfo["theme"] ?>/info.png" width="14" height="14" border="0" alt=""> Canned responses created here will be available just for your account.</div>
			...
			<div style="margin-top: 25px;">
				...
				<div>Make the canned response available for Department:</div>
				...
				<div><textarea name="message" id="message" class="input_text" rows="7" style="white-space: pre-wrap; min-width: 98%; resize: vertical;" wrap="virtual" spellcheck="true"></textarea></div>

				<div style="margin-top: 10px;"><button type="button" onClick="do_submit()" class="input_op_button">Submit</button> &nbsp; &nbsp; &nbsp; <span style="text-decoration: underline; cursor: pointer;" onClick="toggle_new(0)">close</span></div>
				</form>
			</div>
		</td>
	...
	<table cellspacing=0 cellpadding=0 border=0 width="98%" id="table_vis_idle" style="display: none;">
	<tr>
		<td valign="top" nowrap width="100%">
			<div style=""><img src="../themes/<?php echo $opinfo["theme"] ?>/timer.png" width="14" height="14" border="0" alt=""> Visitor idle automatic close chat session.</div>

			<div style="margin-top: 25px;">
				<table cellspacing=0 cellpadding=2 border=0>
				<tr>
					<td style="padding-top: 0px;">After sending the canned response</td>
					<td style="padding-top: 0px;">
						<select style="width: 220px;" id="canned_select_1">
						<?php
							$deptid = 0 ;
							...
							for ( $c = 0; $c < count( $cans ); ++$c )
							{
								...
								$selected = ( isset( $vis_idle_canned["canid_1"] ) && ( $vis_idle_canned["canid_1"] == $can["canID"] ) ) ? "selected" : "" ;
								$cans_string .= "<option value=\"$can[canID]\" $selected>$title_display</option>" ;
							}
							print $cans_string ;
						?>
						</select>
				...
				<tr>
					<td>&rarr; if the visitor does not reply within</td>
					<td>
						<select id="vis_idle_minutes">
						<?php
							for ( $c = 1; $c <= 10; ++$c )
							{
								$selected = ( isset( $vis_idle_canned["idle"] ) && ( $vis_idle_canned["idle"] == $c ) ) ? "selected" : "" ;
				...
				<tr>
					<td>&rarr; send the canned response</td>
					<td>
						<select style="width: 220px;" id="canned_select_2">
						<?php
							$deptid = 0 ;
							$cans_string = "<option value=0>&nbsp;</option>" ;
							for ( $c = 0; $c < count( $cans ); ++$c )
							{
								...
								$selected = ( isset( $vis_idle_canned["canid_2"] ) && ( $vis_idle_canned["canid_2"] == $can["canID"] ) ) ? "selected" : "" ;
								$cans_string .= "<option value=\"$can[canID]\" $selected>$title_display</option>" ;
							}
							print $cans_string ;
						?>
						</select> and automatically close the chat session.
					</td>
				</tr>
				<tr>
					<td></td>
					<td style="padding-top: 25px;"><button type="button" onClick="do_submit_vis_idle()" class="input_op_button">Submit</button> &nbsp; &nbsp; &nbsp; <span style="text-decoration: underline; cursor: pointer;" onClick="toggle_vis_idle(0)">close</span></td>
				</tr>
				</table>
			</div>
		</td>
