...
<!--
	...
	$(document).ready(function()
	{
		...
		else
		{
			...
			else
			{
				...
				else if ( ( dn == 2 ) && ( dn_browser == "chrome" ) ) { $('#dn_chrome').show() ; }
	...
	function sms_send()
	{
		...
		else
		{
			...
			$.ajax({
				...
				data: "action=sms_send&wp="+wp+"&auto=<?php echo $auto ?>&console=<?php echo $console ?>&phonenum="+phonenum+"&carrier="+carrier+"&"+unixtime(),
	...
	function sms_verify()
	{
		...
		else
		{
			$.ajax({
				type: "POST",
				url: "../ajax/chat_actions_op_ext.php",
				data: "wp="+wp+"&auto=<?php echo $auto ?>&console=<?php echo $console ?>&action=sms_verify&code="+code+"&"+unixtime(),
				success: function(data){
					eval(data) ;
					...
					if ( json_data.status )
					{
						...
						{
							if ( typeof( parent.isop ) != "undefined" ) { parent.sms_enabled = 1 ; }
							location.href = "./notifications.php?action=success&jump=sms&wp="+wp+"&auto=<?php echo $auto ?>&console=<?php echo $console ?>" ;
						}
						else
						{
							if ( typeof( parent.isop ) != "undefined" ) { parent.sms_enabled = 0 ; }
							$('#code').val("") ;
	...
	function sms_toggle( theflag )
	{
		...
		$.ajax({
			type: "POST",
			url: "../ajax/chat_actions_op_ext.php",
			data: "action=sms_verify&wp="+wp+"&auto=<?php echo $auto ?>&console=<?php echo $console ?>&code="+theflag+"&"+unixtime(),
			success: function(data){
				eval(data) ;

				if ( json_data.status )
				{
					if ( typeof( parent.isop ) != "undefined" ) { parent.sms_enabled = theflag ; }
	...
	function reset_sms()
	{
		...
		{
			if ( typeof( parent.isop ) != "undefined" ) { parent.sms_enabled = 0 ; }
			location.href = "./notifications.php?action=mreset&jump=sms&wp="+wp+"&auto=<?php echo $auto ?>&console=<?php echo $console ?>" ;
		}
	}
...
<?php include_once( "./inc_header.php" ); ?>
		...
		<div class="op_submenu_wrapper">
			...
			<?php if ( $opinfo["sms"] ): ?><div class="op_submenu" onClick="show_div('sms')" id="menu_sms">SMS Alert</div><?php endif ; ?>
			<div style="clear: both"></div>
		</div>

		<div id="op_sounds" style="display: none; margin-top: 15px;">
			<?php if ( !$console ): ?><div style="margin-bottom: 25px;"><img src="../pics/icons/audio_big.png" width="48" height="48" border="0" alt=""> If the operator chat console is open, you will need to logout of the console and login again to see the changes.</div><?php endif ; ?>
		...
		<div id="op_sms" style="display: none; margin-top: 15px;">
			<img src="../pics/icons/mobile_alert_big.png" width="48" height="48" border="0" alt=""> Receive new chat request SMS alerts on your mobile. This is a notification only. The operator console should remain opened on the computer.
									...
									<tr>
										<td colspan=3>
											<div style="margin-top: 15px;">Verification of your mobile device is required.  There is a 1 minute delay between each verification attempts.  If you do not receive the verification code within 5-10 minutes, double check the mobile number and the carrier values for accuracy.  <a href="https://www.google.com/search?q=List%20of%20Email%20to%20SMS%20Gateways" target="_blank">(Google search: list of email to SMS gateways)</a>
												<div id="sma_reset" style="display: none; margin-top: 25px; text-decoration: underline; cursor: pointer;" onClick="reset_sms()">
													<img src="../pics/icons/reset.png" width="16" height="16" border="0" alt=""> Clear and reset the SMS information.
												</div>
											</div>
										</td>
									</tr>
		...
		<div id="op_dn" style="display: none; margin-top: 15px;">
			<img src="../pics/icons/computer_big.png" width="48" height="48" border="0" alt=""> Display a new chat request/response notification on the desktop.  Desktop notification is currently available for <a href="https://www.google.com/chrome" target="new">Google Chrome</a> and <a href="http://www.firefox.com" target="new">Firefox</a>.
			...
			<tr>
				<td >
					...
					<div id="dn_chrome" style="display: none;">
						<span class="info_error">Desktop notification is not available on Google Chrome non-secure URL.</span>

						<div style="margin-top: 25px;">For Google Chrome browser, the desktop notification will only function on HTTPS secure URLs.</div>
