<?php
	...
	LIST( $ip, $vis_token ) = Util_IP_GetIP( "" ) ; $now = time() ; $ses = "" ;

	$agent = isset( $_SERVER["HTTP_USER_AGENT"] ) ? $_SERVER["HTTP_USER_AGENT"] : "&nbsp;" ;
	LIST( $os, $browser ) = Util_Format_GetOS( $agent ) ;
	$mobile = ( $os == 5 ) ? 1 : 0 ;

	$error = $reload = $password_new = $auto_login_token = $auto_login_token_ses = "" ;
	if ( !isset( $CONF["screen"] ) ) { $CONF["screen"] = "same" ; }
	if ( $auto || $wp || $wpress || ( $query == "op" ) ) { $CONF["screen"] = "separate" ; }
	if ( !isset( $_COOKIE["cK"] ) ) { setcookie( "cK", 1, $now+(60*60*24*90), "/" ) ; }
	...
	if ( is_file( "$CONF[CONF_ROOT]/patches/167" ) && !is_file( "$CONF[CONF_ROOT]/patches/167__" ) )
	{
		...
		unlink( "$CONF[CONF_ROOT]/patches/167" ) ;
		HEADER( "location: patch.php" ) ; exit ;
	}
	if ( $action === "update_auto_login" )
	{
		...
		if ( $value )
		{
			...
			setcookie( "cAT", $auto_login_token, $now+(60*60*24*1095), "/" ) ;
		}
		else { setcookie( "cAT", FALSE, -1, "/" ) ; }
	...
	else if ( $action === "submit" )
	{
		...
		else if ( $menu == "sa" )
		{
			...
			if ( isset( $admininfo["adminID"] ) && ( $password === md5( $admininfo["password"].$token_pass ) ) )
			{
				...
				setcookie( "phplive_adminSES", $ses, -1, "/" ) ;

				database_mysql_close( $dbh ) ;
				HEADER( "location: setup/index.php?$now" ) ; exit ;
		...
		else
		{
			...
			if ( isset( $opinfo["opID"] ) && ( $password === md5( $opinfo["password"].$token_pass ) ) )
			{
				...
				{
					$ses = md5( $now.$ip ) ; $ses = substr( $ses, 0, 10 ) ;
					...
					if ( $mapp )
					{
						...
						Ops_update_OpValue( $dbh, $opinfo["opID"], "mapp", $mapp_opid ) ;
						setcookie( "cO", $opinfo['opID'], $now+(60*60*24*90), "/" ) ;
						setcookie( "cS", $ses, $now+(60*60*24*90), "/" ) ;
					}
					else
					{
						Ops_update_OpValue( $dbh, $opinfo["opID"], "mapp", 0 ) ;
						setcookie( "cO", $opinfo['opID'], -1, "/" ) ;
						setcookie( "cS", $ses, -1, "/" ) ;
					}
					setcookie( "cK", NULL, -1, "/" ) ;

					$ses_filename = $opinfo["opID"]."_ses_$ses" ;
					$dir_files = glob( $CONF["TYPE_IO_DIR"]."/".$opinfo["opID"]."_ses_*", GLOB_NOSORT ) ;
	...
	else if ( $action === "reset_password" )
	{
			...
			if ( $menu == "sa" )
			{
				...
				if ( isset( $admininfo["adminID"] ) )
				{
					...
					else
					{
						...
						$url = "$base_url/index.php?adminid=$admininfo[adminID]&v=".md5( $now.$admininfo["password"] ) ;
			...
			else
			{
				...
				if ( isset( $opinfo["opID"] ) )
				{
					...
					else
					{
						...
						$url = "$base_url/index.php?opid=$opinfo[opID]&v=".md5( $now.$opinfo["password"] ) ;
		...
		database_mysql_close( $dbh ) ; print $json_data ; exit ;
	}
	else if ( $v )
	{
			...
			else if ( ( $action === "submit" ) && ( $menu == "operator" ) && !$error )
			{
				...
				else
				{
					...
					if ( $wp || $auto )
						...
						print "setTimeout( function(){ location.href='ops/index.php?auto=$auto&wp=$wp&$now' ; }, 1500 ) ;" ;
				}
			}
...
<div id="body" style="padding-bottom: 20px;">
	<div style="width: 100%; padding-top: 25px;">
		<div style="<?php echo ( $mapp ) ? "display: none;" : "" ?> width: 280px; margin: 0 auto;">
			<div style="font-size: 14px; color: #C4C4C3; text-shadow: 1px 1px #FFFFFF; text-align: center;" id="chat_text_version"><?php if ( isset( $CONF["KEY"] ) && ( $CONF["KEY"] == md5($KEY."-c615") ) ): ?>Live Chat Solution<?php else: ?>PHP Live! Support<?php endif ; ?> v.<?php echo $VERSION ?></div>
		</div>

		<div style="width: 280px; height: 370px; margin: 0 auto; margin-top: 20px; padding: 10px;">
			...
			<tr>
				<td colspan="2">
					<div style="padding-bottom: 10px;">
						<div id="div_info_operator" style="display: none; padding: 10px; text-align: center;" class="info_blue"><img src="pics/icons/user_big.png" width="16" height="16" border="0" alt=""> Chat Operator Login</div>
						<div id="div_info_setup" style="display: none; padding: 10px; text-align: center;" class="info_blue"><img src="pics/icons/settings_big.png" width="16" height="16" border="0" alt=""> Setup Admin Login</div>
					</div>
				</td>
			</tr>
			<tr>
				<td width="60"><span id="div_txt_login">Login</span></td>
				<td> <input type="text" class="input" name="phplive_login" id="phplive_login" size="15" maxlength="35" value="<?php echo ( $login ) ? $login : "" ?>" onKeyup="input_text_listen(event);" autocomplete="off" style="outline: none;"></td>
			</tr>
			<tr id="div_tr_password">
				<td width="60">Password</td>
				<td> <input type="password" class="input" name="phplive_password_temp" id="phplive_password_temp" size="15" maxlength="35" value="<?php echo ( isset( $password ) && $reload ) ? $password : "" ; ?>" onKeyPress="return noquotes(event)" onKeyup="input_text_listen(event);" autocomplete="off" style="outline: none;"></td>
			</tr>
			<tr>
				<td>&nbsp;</td>
				<td><div style="padding-top: 15px; cursor: pointer;" id="div_remember"><input type="checkbox" name="remember" id="remember" value="1" <?php echo ( $remember ) ? "checked" : "" ; ?>> <span onClick="init_remember_checkbox();">Remember me</span></div></td>
			</tr>
			<tr>
				<td colspan=3>
					<div id="div_btn_submit" style="padding-top: 25px;">
						<button type="button" id="btn_login" onClick="do_login()" class="btn round" style="width: 100%; height: 45px; padding: 6px; font-size: 14px; font-weight: bold; background: #8BCF92; border: 1px solid #7FBD85; color: #FFFFFF;"></button>
					...
					<div id="div_btn_forgot" style="display: none; margin-top: 10px;">
						...
						<div style="margin-top: 15px;">
							<div><button type="button" id="btn_login_forgot" onClick="do_forgot()" class="btn round" style="width: 100%; height: 45px; padding: 6px; font-size: 14px; font-weight: bold; background: #8BCF92; border: 1px solid #7FBD85; color: #FFFFFF;"></button></div>
		...
		<div style="padding-top: 15px;">
			<div id="chat_text_powered" style="width: 280px; margin: 0 auto; font-size: 10px; text-shadow: 1px 1px #FFFFFF; text-align: center;">
				<?php if ( ( isset( $CONF["KEY"] ) && ( $CONF["KEY"] == md5($KEY."-c615") ) ) || $mapp ): ?><?php else: ?>&copy; OSI Codes Inc. - <a href="http://www.phplivesupport.com/?plk=osicodes-5-ykq-m" target="_blank">PHP Live!</a><?php endif ; ?>
			</div>
		</div>
