<?php
	...
	if ( ( $action === "pre_register" ) && $url )
	{
		if ( function_exists( "curl_init" ) && function_exists( "curl_exec" ) )
		{
			...
			if ( ( $response == 0 ) && ( $status != "404" ) && !$curl_errno ) { $error = "X-Frame-Options detected.  For more information please visit the <a href='http://www.phplivesupport.com/r.php?r=xframe' target='_blank' style='color: #FFFFFF;'>X-Frame-Options documentation</a>." ; $json_data = "json_data = { \"status\": 0, \"error\": \"$error\" };" ; }
...
<!--
	...
	function do_register()
	{
		...
		$.ajax({
		type: "POST",
		url: "https://www.phplivesupport.com/mapp/Util/system_register.php",
...
<?php include_once( "../setup/inc_header.php" ) ?>
			...
			<div class="op_submenu_wrapper">
				...
				<?php if ( $admininfo["adminID"] == 1 ): ?><div class="op_submenu" onClick="location.href='../setup/settings.php?jump=profile'" id="menu_profile"><img src="../pics/icons/key.png" width="12" height="12" border="0" alt=""> Setup Profile</div><?php endif ; ?>
			...
			<div id="div_register" style="display: none; margin-top: 25px;">
				To utilize the <a href="http://www.phplivesupport.com/r.php?r=mapp" target="_blank">Mobile Application</a> for Android and iOS, a registration of your PHP Live! system is required.  Registration is required to verify the software License Key and also to generate the mobile app Activation Code.
				...
				<div style="margin-top: 25px;" class="info_info">
					...
					<div style="margin-top: 5px;"><span class="info_box"><?php echo $KEY ?></span></div>
					<div id="div_btn_register" style="margin-top: 25px;">
						...
						<div style="margin-top: 25px;"><button type="button" onClick="init_register()" id="btn_register" class="btn">Generate Mobile App Activation Code</button></div>
						</form>
					</div>
			...
			<div id="div_activated" style="display: none; margin-top: 25px;">
				...
				<div style="margin-top: 25px;" class="info_info">
					...
					<div style="margin-top: 25px; font-size: 18px; font-weight: bold;"><img src="../pics/icons/mobile_big.png" width="48" height="48" border="0" alt=""> Activation Code: <span id="div_kpr" class="info_box"></span> - <span id="div_kpo" class="info_box"></span></div>

					<div style="margin-top: 25px;"><span class="info_misc"><img src="../pics/icons/info.png" width="12" height="12" border="0" alt=""> <b>Reminder:</b></span> <a href="../setup/ops.php">Enable the operator "Mobile App Access"</a> to allow the chat operator to login from the Mobile App.</div>
				</div>
				...
				<div style="margin-top: 25px;">
					...
					<div style="margin-top: 5px;">Automatically set mobile <i>Online</i> operators to <span style="font-weight: bold; color: #D6453D;">Offline</span> status if the operator has <span style="font-weight: bold; color: #D6453D;">not accessed (opened) the mobile application</span> greater then
						...
						<div><img src="../pics/icons/info.png" width="16" height="16" border="0" alt=""> Most Android devices continues to communicate with the server when the Mobile App is placed in the background and is not considered idle state.  This setting is primarily for iOS devices because all network activity is paused when the Mobile App is placed in the background on iOS.</div>
