		...
		for ( var c = 0; c < departments.length; ++c )
		{
			...
			for ( var c2 = 0; c2 < departments[c].operators.length; ++c2 )
			{
				...
				if ( departments[c].operators[c2]["status"] )
				{
					...
					btn_transfer = "<button type=\"button\" class=\"input_op_button\" onClick=\"parent.transfer_chat( "+departments[c]["deptid"]+",'"+departments[c]["name"]+"',"+departments[c].operators[c2]["opid"]+",'"+departments[c].operators[c2]["name"]+"');$(this).attr('disabled', 'true');parent.toggle_extra( 'mapp_vinfo', '', '', 'Visitor Info' )\" style=\"font-size: 12px;\">transfer</button>" ;
				}
