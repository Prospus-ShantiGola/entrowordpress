...
//        ),
    ...
    'subscription_status' => 'IF(CURDATE() > `subscription_end_date`, "Expired", "Active")'
);
    ...
    public function checkUserData($data) {
      ...
      // $condition = array('email_address' => $userEmail, 'password' => $userPass);
        $condition = array( 'password' => $userPass,'OR'=> array(
...
// die;
    ...
    public function getUserShortProfile($userId) {
        $userDetail = $this->find('first', array('conditions' => array('User.id' => $userId), 'fields' => array('id','first_name', 'last_name', 'email_address', 'user_image', 'country_id', 'registration_date', 'gender', 'group_code', 'trail_end_date', 'account_type','username','registration_status','UserTeacherProfile.kbn_number','UserProfile.city','UserProfile.designation','UserProfile.designation','UserTeacherProfile.organization','User.stage_id')));
     
// pr($userDetail);
// die;
        if (count($userDetail)) {
            $detail['id'] = $userDetail['User']['id'];
            ...
            $detail['kbn_number'] = $userDetail['UserTeacherProfile']['kbn_number'];
            $detail['registration_status'] = $userDetail['User']['registration_status'];
            $detail['city'] = $userDetail['UserProfile']['city'];
             $detail['designation'] = $userDetail['UserProfile']['designation'];
              $detail['organization'] = $userDetail['UserTeacherProfile']['organization'];
             $detail['stage_id'] = $userDetail['User']['stage_id'];
            return $detail;
        }
    }
      ...
      public function getAllUserInfo() {
        ...
        //     $conditions = 
        // }
        $userDetail = $this->find('all', array('conditions' => array('User.registration_status'=>'1','UserTeacherProfile.payment_status'=>'Success','User.stage_id !='=>'4' ), 'fields' => array('id','first_name', 'last_name', 'email_address', 'user_image', 'country_id', 'registration_date', 'gender', 'group_code', 'trail_end_date', 'account_type','username','registration_status','UserTeacherProfile.kbn_number','UserProfile.designation','UserProfile.city')));
        // pr($userDetail);
        // // die;
        // die;
        ...
        if (count($userDetail)) {
           ...
           // return $detail;
            ...
            foreach($userDetail as $detaildata)
            {
                $user_id =  $detaildata['User']['id'];
            $detail[$user_id]['id'] = $detaildata['User']['id'];
           $detail[$user_id]['image'] = $detaildata['User']['user_image'];
            $detail[$user_id]['first_name'] = $detaildata['User']['first_name'];
           $detail[$user_id]['last_name'] = $detaildata['User']['last_name'];
            $detail[$user_id]['email_address'] = $detaildata['User']['email_address'];
          $detail[$user_id]['country_id'] = $detaildata['User']['country_id'];
           $detail[$user_id]['registration_date'] = $detaildata['User']['registration_date'];
            $detail[$user_id]['gender'] = $detaildata['User']['gender'];
            $detail[$user_id]['trail_end_date'] = $detaildata['User']['trail_end_date'];
            $detail[$user_id]['group_code'] = $detaildata['User']['group_code'];
            $detail[$user_id]['context_role_id'] = $detaildata['ContextRoleUser']['context_role_id'];
            $detail[$user_id]['context_user_role_id'] = $detaildata['ContextRoleUser']['id'];
           $detail[$user_id]['account_type'] = $detaildata['User']['account_type'];
            $detail[$user_id]['username'] = $detaildata['User']['username'];
            $detail[$user_id]['kbn_number'] = $detaildata['UserTeacherProfile']['kbn_number'];
            $detail[$user_id]['registration_status'] = $detaildata['User']['registration_status'];
           $detail[$user_id]['city'] = $detaildata['UserProfile']['city'];
            $detail[$user_id]['designation'] = $detaildata['UserProfile']['designation'];
             // array_push($final_output, $detail);
 //   pr($detail);
 // die;
            }
    ...
    {
        return $arrContextRole = $this->query('SELECT roles.id as role_id ,roles.role, context_roles.id as context_id, contexts.name as context, context_role_users.id as context_role_user_id,users.user_image FROM users  LEFT JOIN context_role_users   ON  users.id = context_role_users.user_id LEFT JOIN context_roles  ON context_role_users.context_role_id = context_roles.id LEFT JOIN contexts  ON context_roles.context_id = contexts.id LEFT JOIN  roles  ON context_roles.role_id = roles.id WHERE users.id ='. $user_id);
    }
    
    /**
     * Get user parent by user id. 
     */
    public function getUserParentById(){
