<script type="text/javascript" src="//s7.addthis.com/js/300/addthis_widget.js#pubid=ra-53be7b323b162f50"></script>
...
?>
<div class="official-post-wrapper" data-id= "<?php echo $post['Escene']['id']?>"> 
  ...
  <div class="row view-comment-list">
      ...
      <div class="user-post-deatils col-md-11">
        <p><a href="javascript:void(0)" class="anchor-heading user-name"><?php echo  $post['User']['first_name']." ".$post['User']['last_name'] ; ?></a> 
...
<!--         Start of user detail bind in div   -->
            <div class="tooltips-user">           
                <p><img src="<?php echo $userImage ;?>" alt="">
                    <?php 
                     $role = $this->User->getRoleByUserId($post['User']['id']); 
                    echo  $post['User']['first_name']." ".$post['User']['last_name'] . '('.$role.')' ; 
                    ?>                    
                </p>
            </div>
 <!--       End of user detail bind in div   --> 
        <span class="post-date"><?php echo  date("M j",strtotime($post['Escene']['post_date'])) ." at ". date("g:i a",strtotime($post['Escene']['post_date'])); ?></span> 
       ...
       $image_count =  $this->EsceneComment->getImageCount($post['Escene']['id']);
          ...
          foreach($post['EsceneImage'] as $key=>$postImage){
             ...
             // }
              ...
              ?>
    <div class="row margin-top-small" <?php if($image_count!=1){?> style= "float:left; width:250px;" <?php }?> > 
      <div class="col-md-8" ><img src="<?php echo $imgPath;?>"  alt=""  class="img-thumbnail"> </div>
    </div>
    <?php }
...
<div style="clear:both"></div>
  ...
  <div class="comments-likes-area">
   ...
   // echo $this->element('escene_all_comment_data_element', array('comment'=>$post['EsceneComment']));
//,$this->Session->read('user_id')?>
  </div>
  <div class="add-comment-area background-white">
    <div class="row view-comment-list">
