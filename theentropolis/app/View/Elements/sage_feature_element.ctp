<?php
$arrPost = array(1=> 'Advice Publishing App', 0=>'Feature Blog', 'Entropolis HQ', 'Club Kidpreneur HQ', 'Latest Posts', 'Archived Posts', 'Media and PR', 'Videos');
if(!empty($adv_type)){
    $title = $arrPost[$adv_type];
}
...
<div class="advice_edit profile_detail_view" >
 <div class="inside-profile">
          <h2>
            ...
            <span>
              <?php
              echo $adviceInfoData['ContextRoleUser']['User']['username'];
              //echo $adviceInfoData['ContextRoleUser']['User']['first_name']." ".$adviceInfoData['ContextRoleUser']['User']['last_name'];?>
            </span>
            <!--                <ul class="advice-bdr">
          ...
          <input type ="hidden" value = "<?php echo $adviceInfoData['ContextRoleUser']['User']['username'];?>" class= "sage-name-for">
          ...
          <h4 class="h4tag roboto_medium" title="<?php echo $adviceInfoData['Advice']['advice_title'];?>" ><?php echo trim($adviceInfoData['Advice']['advice_title']); ?><a href="<?php echo $adviceInfoData['Advice']['source_url'];?>" target="_blank"> <?php echo $this->Html->image('svg-icons/link.svg');?></a></h4>
          <span><strong>Category: </strong><?php echo $adviceInfoData['DecisionType']['decision_type'];?></span>
          <span><strong>Published On: </strong><?php echo date('M j, Y',strtotime($adviceInfoData['Advice']['advice_decision_date']));?></span>
        ...
        <div class="col-md-12 ">
            <div class="col-md-12 profile_biography containerHeight custom_scroll">
                <div class="popups_detail_blocks">
                    ...
                    $exSummary = $adviceInfoData['Advice']['feature_blog'];
                    $exSummary = preg_replace('/<!--(.|\s)*?-->/', '', $exSummary);
                    if(strlen($exSummary) > 435)
                    {?>
                  <div class="person-content short-data"><?php 
                                          
                      // echo substr($post['Escene']['post_description'], $remaining-1 );  
                      $actual_lenth = strlen(trim($exSummary)); 
                      $shortData = ($this->Eluminati->text_cut($exSummary, $length = 435, $dots = true)); 
                      if(strpos($shortData, 'href') !== FALSE){
                          
                          $addedStr =  '<div class="original-link"><i> Original Link: </i></div> <a'; 
                          $shortData = str_replace('<a', $addedStr, $shortData);
                      }
                      //$shortData = str_replace('<a', '<a target="_blank"', $shortData);
                      //$shortData = str_replace('< a', '<a target="_blank"', $shortData);
                      echo $shortData;
                      $later_length =  strlen(trim($this->Eluminati->text_cut($exSummary, $length = 435, $dots = true)));?>
                    </b></strong></a></em></i></div>
                    <div class="person-content full-data hide"  data-to="1"> 
                  <?php $fullStr =  ($exSummary); 
                        if(strpos($fullStr, 'href') !== FALSE){
                          
                          $addedStr =  '<div class="original-link"><i> Original Link: </i></div> <a'; 
                          $fullStr = str_replace('<a', $addedStr, $fullStr);
                        }
                        //$fullStr = str_replace('<a', '<a target="_blank"', $fullStr);
                        //$fullStr = str_replace('< a', '<a target="_blank"', $fullStr);
                        echo $fullStr;
                        ?>
                    </div>  
                      ...
                      <?php 
                      $fullStr = ($this->Eluminati->text_cut($exSummary, $length = 435, $dots = true));
                      if(strpos($fullStr, 'href') !== FALSE){
 ...
 <div class="bottom-wrap">
              ...
              <div class = 'share-button-image'> </div>
                ...
                <div class="show-detail invitation">
                    ...
                    <h5>Hi there <b class= "modal-sage-name"><?php echo $adviceInfoData['ContextRoleUser']['User']['username'];//$adviceInfoData['ContextRoleUser']['User']['first_name'];?></b>,<span><i class="icons close-grey-icon"></i></span></h5>
                    <span>
                ...
                <div class="show-detail mail">
                    ...
                    <div class="mail-wrap btn-yellow">
                        <div class="entry">
                            <label>To:</label>
                            <span class = "modal-sage-name"><?php echo  $adviceInfoData['ContextRoleUser']['User']['username'];//$adviceInfoData['ContextRoleUser']['User']['first_name'];?></span>                        
                        </div>
                        <div class="entry">
                            <label>From:</label>
...
<!--                    // Edit Delete stuff here-->
                        <a href="javascript:void(0)" class="edit-advice data-loading" data-advice-type="<?php echo $adv_type; ?>" data-id = "<?php echo $adviceInfoData['Advice']['id']; ?> "><i class="icons edit-white"></i></a>
                        <a href="javascript:void(0)" class="delete-advice-data"><i class="icons delete-white"></i></a>
                         <a href="#" data-open="comment"><i class="icons comment"></i></a>  
                        <a href="#" data-open="share" data-type = "Advice"  class = "share-button disabled" data-id = "<?php echo $adviceInfoData['Advice']['id']; ?>"><i class="icons share"></i></a>
<!--                        <a href="#" data-open="mail"><?php //echo $this->Html->image('black-icon5.png');?></a>-->
                    ...
                    <?php
                        ...
                        ?>
                        <a href="#" data-open="comment"><i class="icons comment"></i></a>
                        <a href="#" data-open="rate"><i class="icons star"></i></a>
                        <a href="#" data-open="share" data-type = "Advice"  class = "share-button disabled" data-id = "<?php echo $adviceInfoData['Advice']['id']; ?>"><i class="icons share"></i></a>
                        ...
                        <a class = "attach-library" data-type = "Advice" data-owner = "<?php echo $adviceInfoData['ContextRoleUser']['user_id'];?>" data-id = "<?php echo $adviceInfoData['Advice']['id']; ?> "><i class="icons library"></i></a>
              <?php } ?>
                    <?php 
                    if($this->Session->read('context_role_user_id') == $adviceInfoData['Advice']['context_role_user_id']){ ?>
<!--                        <a href="#" data-open="attach"><?php echo $this->Html->image('black-icon6.png');?></a> -->
                   <?php
                    ...
                    <!--   <a href="#" data-open="rate"><img src="images/black-icon2.png" alt=""></a>  -->    <a href="<?php echo Router::url(array('controller'=>'users', 'action'=>'login'));?>"><i class="icons star"></i></a>
...
//alert(slideIndex);
            ...
            var currentContainerId = $(slider).closest('.modal').attr('id') ? $(slider).closest('.modal').attr('id') : 'sageadvice-popup';
...
//# sourceURL=sage_feature_element.ctpjs
</script>    
