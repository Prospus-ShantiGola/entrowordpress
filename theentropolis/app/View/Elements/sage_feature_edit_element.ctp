<div class="row advice_user_id test1" data-userid = "<?php echo $adviceInfoData['ContextRoleUser']['User']['id'] ?>" data-context = "<?php echo $adviceInfoData['ContextRoleUser']['id'] ?>" >
    ...
    <div class="col-lg-12">
        <div class="profile_detail relative">
            <div class="profile-img-blck profile_img"> <?php 
               ...
               $adv_type = '';
                ...
                else if($adviceInfoData['Advice']['executive_summary']) {
                  $adv_type = 'Blog';
     ...
     $img = $this->Common->getRoleIcon($adviceInfoData['ContextRoleUser']['user_id']);
     $img = $this->Html->image($img);
      ?>
            ...
            <div class="advice_edit profile_detail_view custom_scroll">
              ...
              <span class="txt-wrapper clearfix"><strong class="txt-left">Category:</strong>
                <!-- <select name="decision_type_id" class="decision-types form-control">
                  <?php
                  $i = 0;
                  foreach($decisionTypes as $key=>$decType){ $i++;?>
                  ...
                  <?php echo $this->Form->input('decision_type_id', array('options' => $decisionTypes, 'id' => 'decision_type_id', 'class' => 'decision-types form-control m-b-10 width-100', 'label' => false, 'empty' => 'Category*', 'data-toggle' => 'popover', 'data-placement' => 'bottom','value' => $adviceInfoData['DecisionType']['id'])); ?>
                <span class="error-message"> </span>
              </span>
        ...
        <div class="col-md-12 ">
            <div class="col-md-12 profile_biography containerHeight custom_scroll">
                <div class="popups_detail_blocks">
                    <h4 class="small_title">Feature Blog</h4>
                    <?php  $exSummary = $adviceInfoData['Advice']['feature_blog'];?>
                      <div class="person-content short-data txt-wrapper">
<!--                          <div class="editable executive-summary"><?php //echo $exSummary;?></div>-->
                         <textarea name="editor1" class="feature-blog ckeditor" id="feature-blog" rows=""><?php echo $exSummary;?></textarea>
...
<!--                                <div class="doc-wrap">
                ...
                <div class="popups_detail_blocks">
                    ...
                    <?php
                    echo nl2br($this->Eluminati->text_cut($adviceInfoData['Advice']['key_advice_points']));
                    ?>
                </div>
            </div>
        </div>
            ...
            <div class="bottom-wrap">
              <div class="show-detail attach">
                   <h5 class="roboto_medium">Images/Documents<span><i class="icons close-icon"></i></span></h5>
            ...
            <div class="modal-footer align-center modal-bottom-strip ">
                ...
                if($this->Session->read('context_role_user_id') == $adviceInfoData['Advice']['context_role_user_id']){ ?>                 
                    ...
                    <button name="publish" class="btn btn-black grey_button publish-advice ">PUBLISH </button>
                    <button name="cancel" data-advice-type="<?php echo $adv_type;?>" data-id="<?php echo $adviceInfoData['Advice']['id'];?>" data-type="Advice" class="btn grey_button btn-black cancel-advice get-new-modal set-data-advice ">CANCEL </button>
...
$(function() {
    $('body').on('change', '.decision-types', function () {
           getSubCategory(this, '<?php echo $this->webroot ?>challengers/decision_category/', '.category-id', '.advice_edit, .txt-wrapper');
    });
});
...
//# sourceURL=sage_feature_edit_element.ctpjs
</script>