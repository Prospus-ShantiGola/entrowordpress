    ...
    jQuery.ajax({
      ...
      {    
          ...
          if(type =='post')
          {
            ...
            //var html_div = "<a href='javascript:void(0);' class= 'like-management' data-type= 'post' data-likeid = 'like-'"+data+"'' data-id ='escene-'"+obj_id+"'' >Unlike</a>';

            var html_div = "<a href='javascript:void(0);' class= 'unlike-management' data-type= 'post' data-likeid='like-"+result[0]+"' data-id ='escene-"+obj_id+"'>Unlike</a>";
            if(count==1)
            {
              var first_user = getUserShortprofile(result[4]);
              
               var new_html ="<p class='post-count-data'><a href='javascript:void(0);' class='user-name' data-toggle='popover' data-placement='bottom' data-html='true' data-content='"+first_user+"'>"+result[2]+"</a> <acronym title='Like'><i class='fa fa-thumbs-up'></i></acronym> this.</p>";
            }
            else if(count==2)
            {
              var first_user = getUserShortprofile(result[4]);
          ...
          //case of comment
            ...
            var html_div = "<a href='javascript:void(0);' class= 'unlike-management' data-type= 'comment' data-likeid = 'like-"+result[0]+"' data-id ='comment-"+obj_id+"'>Unlike</a> ";
             ...
             //  alert("cc"+count);
                //console.log(obj);
                var add_html = "<div class='tooltips'></div><span class='comment-like-count'><acronym title='Unlike'><i class='fa fa-thumbs-up'></i></acronym>&nbsp; <a href='javascript:void(0);' data-startlimt='0' data-likecount='"+count+"' data-id='comment-"+obj_id+" data-type='comment' class= 'like-count show-comments'>"+count+"</a></span>";
         ...
         {  
          ...
          //  alert(count);
            if(type =='post')
            {
              ...
              var html_div = "<a href='javascript:void(0);' class= 'like-management' data-type= 'post' data-likeid='' data-id ='escene-"+obj_id+"'>Like</a>";
            if(count==0)
            {
               obj.parent().parent().parent().siblings('.comments-likes-area').find('.post-count-data').remove();
            }
            ...
            {
                obj.parent().parent().parent().siblings('.comments-likes-area').find('.post-count-data').remove();
               var new_html = "<p  class='post-count-data'><a href='javascript:void(0);' class='user-name' data-toggle='popover' data-placement='bottom' data-html='true' data-content='"+first_user+"'>"+result[2]+"</a> and <a href='javascript:void(0);' data-type='post' data-id = 'escene-"+obj_id+"' data-likecount='"+actual_like_count+"' data-startlimt='0' class='show-comments'>"+actual_like_count+"</a> others <acronym title='Like'><i class='fa fa-thumbs-up'></i></acronym> this.</p>";
            }
            obj.parent().parent().parent().siblings('.comments-likes-area').prepend(new_html);
 $('.user-name').popover(); 
            ...
            object.fileupload({
                ...
                add: function (e, data) {
                    ...
                    if (uploadFile.size > 5000000) { // 5mb
                        // alert message
                        bootbox.alert("Please upload a smaller image, max size is 5 MB.2");
                        goUpload = false;
                    }
                    if (goUpload == true) {                                 
                        var img = data.submit();
